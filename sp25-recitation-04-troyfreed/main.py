...
def sentiment_map(doc,
    ...
    doc = doc.split()
    map = []
    for test_word in doc:
        for pos_word in pos_terms:
            if test_word == pos_word:
                tuple_pos = ('positive', 1)
                map.append(tuple_pos)
        for neg_word in neg_terms:
            if test_word == neg_word:
                tuple_neg = ('negative', 1)
                map.append(tuple_neg)
    return map
