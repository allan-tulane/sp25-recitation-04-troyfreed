...
The work would be O(n) because it sums up the word n times. Making the total work linear to the amount
of times the word shows up.

The span would be O(log(n)) because it is a parallel reduction that usually uses a binary tree structure.
In each step the function divides the list, and because it is does in parallel the longest dependent function
is O(log(n))
...
The problem is that race conditions can occur when using a dictionary while trying to parallelize a solution.
Each subset would be trying to update the dictionary at the same time which can cause problems.
To avoid this it is much more efficient to divide and work on each subset separately then merge the answers in the end.
